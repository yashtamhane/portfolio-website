...
function toBio() {
  var element = document.getElementById("bio");  // Target the "bio" section
  var headerOffset = 85;  // Adjust this value if your header is fixed or has a different height
  var elementPosition = element.getBoundingClientRect().top;
  var offsetPosition = elementPosition + window.pageYOffset - headerOffset;
...
const loadingMessages = [
  "Our tech elves are working their magic… Please hold on!",
  "A wizard is casting the final spell… Almost there!",
  "The data train is on the tracks, heading your way!",
  "A hero is being summoned… Your journey is about to begin!",
  "The scrolls of data are being read… Your adventure awaits!",
  "The treasure chest is being unlocked… Get ready for something amazing!",
  "The ancient spell is being cast… You'll be transported soon!",
  "A dragon is flying through the clouds… Hold tight, we're almost there!",
  "The wizard's potion is brewing… Just a little longer!",
  "The ship is setting sail… Prepare for an unforgettable voyage!",
  "A treasure map is being drawn… The hunt begins soon!",
  "The crew is gathering the gold… Ready for an adventure on the high seas?"
];

// Function to get a random starting index
function getRandomStartIndex() {
  return Math.floor(Math.random() * loadingMessages.length);
}

// Create a new Typewriter instance with the circular array logic
let randomIndex = getRandomStartIndex();
new Typewriter(".loading-text", {
  strings: [loadingMessages[randomIndex]],
  autoStart: true,
  delay: 20,
  cursor: "|",
  deleteSpeed: 10,
  onStringTyped: () => {
    // Once the current message finishes typing, load the next message in the array (circular)
    randomIndex = (randomIndex + 1) % loadingMessages.length;
    setTimeout(() => {
      // Restart the typing effect with the new message
      new Typewriter(".loading-text", {
        strings: [loadingMessages[randomIndex]],
        autoStart: true,
        delay: 50,
    ...
    }, 1000); // Delay to switch to the next message
  }
});
...
const projectData = [
  {
    ...
    description:
      "This network sniffing tool captures and monitors network traffic in real-time, helping identify vulnerabilities and unauthorized activity. Its simple interface allows for easy analysis and troubleshooting of network communications.",
    techStack: ["Python"],
    srcURL: "https://github.com/yashtamhane/network-sniffing-tool",
  },
  {
    title: "Multiplayer Chess",
    description:
      "This project implements a basic networked Chess game where two players connect, with the server maintaining the game state and the client synchronizing moves. It focuses on parsing chess moves and processing FEN strings, excluding complex rules like castling and en passant.",
    techStack: ["C++", "C"],
    srcURL: "https://github.com/yashtamhane2026/cse220_hw4",
  },
  {
    title: "Dynamic Upwords",
    description:
      "This project implements functions for a variant of Upwords, allowing tiles to form legal words on a dynamically growing board. It supports horizontal and vertical word placement and expands the board as needed. The project uses a dictionary to validate words, excluding features like scoring or game-ending conditions.",
    techStack: ["C++", "C"],
    srcURL: "https://github.com/yashtamhane2026/cse220_hw3",
  },
    ...
    elem.innerHTML = buildTemplate(
      `<div class='projects-header'>
       ...
       <div class="card-buttons">
          <a href="{srcURL}" target="_blank" class="card-btn" style="float: right; color: #fff; background-color: #9333ea; padding: .375rem .75rem; border-radius: .25rem;"><i class="fa-solid fa-display"></i>Source Code</a>
      </div>`,
      data
    );
    setTimeout(() => {
